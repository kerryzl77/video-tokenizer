...
def get_logits(i3d, videos, device, bs=10):
    model_device = next(i3d.parameters()).device      # cuda:0 or cpu

    feats = []
    with torch.no_grad():
        for idx in range(0, videos.size(0), bs):
            batch = videos[idx:idx+bs].to(model_device, non_blocking=True)

            # forward to Mixed_5c (1024‑D) rather than final 400‑logits
            if isinstance(i3d, torch.nn.DataParallel):
                f = i3d.module.extract_features(batch)   # [B,1024,T,1,1]
            else:
                f = i3d.extract_features(batch)

            f = f.squeeze(-1).squeeze(-1).mean(dim=2)    # → [B,1024]
            feats.append(f.cpu())                        # keep on CPU
